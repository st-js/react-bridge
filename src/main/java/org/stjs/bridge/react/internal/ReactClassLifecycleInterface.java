...
public interface ReactClassLifecycleInterface<P extends Props, S extends State> {
	/**
	 * Invoked when the component is initially created and about to be mounted. This may have side effects, but any external subscriptions or data created by
	 * this method must be cleaned up in `componentWillUnmount`.
	 */
	void componentWillMount();

	/**
	 * Invoked when the component has been mounted and has a DOM representation. However, there is no guarantee that the DOM node is in the document.
	 *
	 ...
	 *
	 * shouldComponentUpdate: function(nextProps, nextState, nextContext) { return !equal(nextProps, this.props) || !equal(nextState, this.state) ||
